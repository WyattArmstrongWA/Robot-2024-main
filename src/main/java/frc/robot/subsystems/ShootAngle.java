...
public class ShootAngle extends SubsystemBase {
    ...
    private final TalonFX angleMotor = new TalonFX(15);
...
public void ShootAnglePeriodic() {
    m_mechanism.update(angleMotor.getPosition());
}
...
public void ShootAngleSet(double angle) {
    ...
    angleMotor.setControl(m_voltagePosition.withPosition(-desiredRotations));
}

public void ShootAngleChange(double change) {
    desiredRotations+=change;
    if (desiredRotations > MAX_ANGLE){
        desiredRotations = MAX_ANGLE;
